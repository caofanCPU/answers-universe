...
- `question-answer-options` 中的"标记正确 / 删除选项 / 展开收起"就是局部控制按钮。
...
- `question-list-filters` 里的 ID / UUID 清空按钮就是典型样例。
...
- `question-list-client` 里的文本分页按钮适合收口到 `XButton single subtle`。
- `question-editor-client`、`random-question-board-client` 里的左右箭头按钮不适合替换成普通 CTA。
...
- `random-question-board-client` 中的月历日期格子应继续视为"日历选择单元"。
- 如果未来要统一，应做 `CalendarCellButton` 或直接使用日历组件体系，不应归入普通 CTA。
