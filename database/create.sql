-- 用户表
CREATE TABLE IF NOT EXISTS faq.users (
...
ON faq.users (clerk_user_id) 
WHERE status <> 'deleted'; 

CREATE UNIQUE INDEX IF NOT EXISTS users_stripe_cus_id_key 
ON faq.users (stripe_cus_id) 
WHERE status <> 'deleted';

CREATE INDEX IF NOT EXISTS idx_users_fingerprint_id ON faq.users (fingerprint_id);

-- 订阅表
CREATE TABLE IF NOT EXISTS faq.subscriptions (
    id                   BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_subscriptions_pay_subscription_id ON faq.subscriptions (pay_subscription_id);
CREATE INDEX IF NOT EXISTS idx_subscriptions_order_id ON faq.subscriptions (order_id);
CREATE INDEX IF NOT EXISTS idx_subscriptions_user_id ON faq.subscriptions (user_id);

-- 积分表
CREATE TABLE IF NOT EXISTS faq.credits (
    id                        BIGSERIAL PRIMARY KEY,
    user_id                   UUID        NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_credits_user_id ON faq.credits (user_id);

-- 交易订单表
CREATE TABLE IF NOT EXISTS faq.transactions (
    id                   BIGSERIAL PRIMARY KEY,
    user_id              UUID         NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_transactions_order_id ON faq.transactions (order_id);
CREATE INDEX IF NOT EXISTS idx_transactions_pay_subscription_id ON faq.transactions (pay_subscription_id);
CREATE INDEX IF NOT EXISTS idx_transactions_user_id ON faq.transactions (user_id);


-- 积分使用审计表
CREATE TABLE IF NOT EXISTS faq.credit_audit_log (
    id               BIGSERIAL PRIMARY KEY,
    user_id          UUID         NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_credit_audit_log_credit_type ON faq.credit_audit_log (credit_type);
CREATE INDEX IF NOT EXISTS idx_credit_audit_log_operation_type ON faq.credit_audit_log (operation_type);
CREATE INDEX IF NOT EXISTS idx_credit_audit_log_user_id ON faq.credit_audit_log (user_id);


-- 用户信息备份表
CREATE TABLE IF NOT EXISTS faq.user_backup (
    id                BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_user_backup_clerk_user_id ON faq.user_backup (clerk_user_id);
CREATE INDEX IF NOT EXISTS idx_user_backup_email ON faq.user_backup (email);
CREATE INDEX IF NOT EXISTS idx_user_backup_fingerprint_id ON faq.user_backup (fingerprint_id);
CREATE INDEX IF NOT EXISTS idx_user_backup_original_user_id ON faq.user_backup (original_user_id);


-- 第三方对接日志审计表
CREATE TABLE IF NOT EXISTS faq.apilog (
    id            BIGSERIAL PRIMARY KEY,
